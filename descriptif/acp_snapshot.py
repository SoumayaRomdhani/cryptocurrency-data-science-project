...
def display_acp_snapshot(df: pd.DataFrame) -> None:
    st.subheader("Analyse en Composantes Principales — Structure du marché crypto")
    ...
    # Distance au centre (spécificité)
    ...
    st.markdown("###  Cartographie structurelle du marché (individus)")
    ...
    if {"PC1", "PC2"}.issubset(pca_df.columns):
        ...
        # Annoter seulement les cryptos les plus spécifiques
    ...
    # 6) CERCLE DE CORRÉLATION (VARIABLES)
    ...
    if X.shape[1] >= 2 and n_components >= 2:
        ...
        st.caption(
            ...
            "Même direction = corrélation positive, opposée = corrélation négative, perpendiculaire ≈ indépendance."
        )
    else:
